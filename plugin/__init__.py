...
required_anki_version = (23, 10, 1) # tested minimum version working, original 23.10(.0) fails
...
class AnkiConnect:
    ...
    @util.api()
    def getCollectionConfig(self):
        col = self.collection()
        global_config = col.all_config()
        return global_config

    @util.api()
    def isFsrsActive(self):
        col = self.collection()
        # Import Config if not already imported
        from anki.config import Config
        global_config = col.all_config()
        #Get fsrs setting, default to None if not found
        fsrs_setting = global_config.get('fsrs', None)
        return fsrs_setting
    ...
    @util.api()
    def createModel(self, modelName, cardTemplates, inOrderFields, css = None, isCloze = False):
        ...
        for field in inOrderFields:
            if isinstance(field, str):
                ...
                if 'description' in field:
                    fm['description'] = field['description']
                if 'collapsed' in field:
                    fm['collapsed'] = field['collapsed']
                mm.addField(m, fm)
